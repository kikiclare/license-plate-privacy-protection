...
from os.path 				import splitext, basename, isdir, join, split
	...
	try:
		...
		for i,img_path in enumerate(imgs_paths):
			...
			if len(R):
				...
				Korig = cv2.imread(join(kpath, split(img_path)[1]))
				...
				for i,r in enumerate(R):
					...
					Kcar = crop_region(Korig,label)

					Lcars.append(label)

					cv2.imwrite('%s/%s_%dcar.png' % (output_dir,bname,i),Icar)
					cv2.imwrite('%s/%s_%dcar-k.png' % (output_dir,bname,i),Kcar)
