...
for img_file in img_files:
	...
	if Lcar:
		...
		for i,lcar in enumerate(Lcar):
			...
			lp_car		 	= '%s/%s_%dcar.png'		    % (output_dir,bname,i)
			Icar		 	= cv2.imread(lp_car)
			car_size		= Icar.shape	# car_size[0]: heigth	car_size[1]: width	car_size[2]: channels
			...
			if isfile(lp_label):
				...
				if isfile(lp_label_str):
					...
					sys.stdout.write(',NoLPrecognized')

				sys.stdout.write('_%s' % car_size[1])
