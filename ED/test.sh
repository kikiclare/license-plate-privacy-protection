...
usage() {
	...
	echo "   -k   Path to K-fuzz images"
	echo "   -h   Print this help information"
	echo ""
	exit 1
}

while getopts 'i:o:c:l:k:h' OPTION; do
	case $OPTION in
		i) input_dir=$OPTARG;;
		o) output_dir=$OPTARG;;
		c) csv_file=$OPTARG;;
...
if [ -z "$kpath"   	  ]; then echo "K-fuzz not set."   ; usage; exit 1; fi
...
# Detect vehicles
python vd4test.py $input_dir $output_dir $kpath

# Detect license plates
python lpd4test.py $output_dir $lp_model 

# OCR
python license-plate-ocr.py $output_dir

# Draw output and generate list
python go4test.py $input_dir $output_dir > $csv_file

# Clean files and draw output
rm $output_dir/*_lp.png
rm $output_dir/*car.png
rm $output_dir/*car-k.png
