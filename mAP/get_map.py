...
parser.add_argument('--source', nargs=1, type=str, help="Position of picture.")
parser.add_argument('--mapto', nargs=1, type=str, help="mAP output.")
parser.add_argument('--mapPngTo', nargs=1, type=str, help="mAP PNG output.")
args = parser.parse_args()

IMG_PATH = args.source[0] # 图片路径
'''
    0,0 ------> x (width)
     |
     |  (Left,Top)
...
with open(results_files_path + "/results.txt", 'w') as results_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        '''
        with open(results_files_path + '/AP.csv', 'a') as csvfile:
    ...
    results_file.write(text + "\n")
    if args.mapto is not None:
        with open(args.mapto[0], "w", encoding='utf-8')as f:
            f.write(str(mAP))
    print(text)
...
for class_name in dr_classes:
    # if class exists in detection-result_t but not in ground-truth then there are no true positives in that class
...
if draw_plot:
    ...
    if args.mapPngTo is None:
        output_path = results_files_path + "/mAP.png"
    else:
        output_path = args.mapPngTo[0]
    to_show = False
    plot_color = 'royalblue'
    draw_plot_func(
        ap_dictionary,
